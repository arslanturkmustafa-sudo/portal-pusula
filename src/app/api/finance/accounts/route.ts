...
import {
  ...
  FinanceAccountIdempotencyConflictError,
...
export async function GET(request: NextRequest): Promise<NextResponse> {
  if (!(await authenticateAdminRequest(request, "finance.accounts.read"))) {
    return spendingJson({ status: "unauthorized" }, 401);
  }
  const correlationId = correlationIdFromHeaders(request.headers);
  try {
    return spendingJson(await listFinanceAccountsOverview(spendingDatabasePool()));
  } catch (error) {
    const mysqlErrorCode = safeMySqlErrorCode(error);
    requestLogger(correlationId).error(
      { event: "finance_account.api.list_failed", mysqlErrorCode },
      `Finance account list failed: ${mysqlErrorCode}`,
    );
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  ...
  const correlationId = correlationIdFromHeaders(request.headers);
  try {
    const input = createFinanceAccountInputSchema.parse(
      await readSpendingBody(request),
    );
    const result = await createFinanceAccount(spendingDatabasePool(), input, {
    ...
    if (error instanceof FinanceAccountIdempotencyConflictError) {
      return spendingJson({ status: "idempotency_conflict" }, 409);
    }
    const mysqlErrorCode = safeMySqlErrorCode(error);
    requestLogger(correlationId).error(
      { event: "finance_account.api.create_failed", mysqlErrorCode },
      `Finance account create failed: ${mysqlErrorCode}`,
    );
    return spendingJson({ status: "service_unavailable" }, 503);
  }
}
