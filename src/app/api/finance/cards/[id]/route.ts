...
export async function PATCH(
  ...
  try {
    ...
    const input = updateCreditCardInputSchema.parse(await readSpendingBody(request));
    const card = await updateCreditCard(spendingDatabasePool(), id, input, {
      actorId: spendingActorId(principal),
      correlationId,
    });
    return spendingJson({ card });
  } catch (error) {
    if (
