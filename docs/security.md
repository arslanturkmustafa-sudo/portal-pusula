...
- `EMAIL_NOTIFICATIONS_ENABLED`, `RESEND_API_KEY`, `EMAIL_FROM_ADDRESS`, `EMAIL_FROM_NAME`;
...
Gerçek değerler repoya, ZIP'e, checkpoint'e, belgeye, sohbete, CLI argümanına, URL'ye, loga, audit payload'ına veya hata metnine yazılmaz. Connection string üretilmez. Readiness ve cron token'ları farklı olmak zorundadır; cron minimum aralığı yalnız canonical `60..86400` saniye olabilir. E-posta anahtarı ve alıcı/gönderici adresleri loglanmaz. E-posta environment'ı eksik veya biçim dışıysa bildirim kapalı kalır; finans kaydının oluşmasını engellemez.
...
Canlı panelde secret gösteren rollback ekranı daha önce gözlendi; değerler kaydedilmeden ekrandan çıkıldı ve ilgili secret'lar kullanıcı tarafından rotasyonla geçersiz kılındı. `2026-09-04` tarihli manuel ZIP yeniden dağıtım akışındaki ayarlar ekranı da mevcut environment değerlerini maskesiz gösterdi. `2026-09-17` tarihinde kullanıcı iki paket yüklemesini Codex'in mevcut ayarlarla tamamlamasını açıkça istedi. Yeniden dağıtım ekranının bazı mevcut değerleri hâlâ açık gösterdiği gözlendi; değerler belgelere veya komutlara taşınmadı. [Dağıtım prosedürüne](./hostinger-deploy.md) göre mevcut değerler değiştirilmeden yükleme yapılabilir; input değerleri DOM'dan çıkarılmaz, parolayı göster kontrolü kullanılmaz ve sır içeren bölüm tekrar görüntülenmez. Yeni kimlik bilgisi girişi ve rotasyon kullanıcıya devredilir. Ayrı canlı rollback kabulü yalnız bu yükleme izniyle tamamlanmış sayılmaz.
