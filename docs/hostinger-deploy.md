...
Bu runbook, `portal.muhendiskafasi.com.tr` Hostinger Business Node.js Web App hedefini kapsar. Güncel proje erişimi geçişinde [0027 kurulum adımları](./phpmyadmin-project-access-incremental.md) izlenir: başlangıç `0026` ve 27 journal kaydı, sonuç `0027` ve 28 journal kaydıdır. Aşağıdaki `0024` DB-first bölümü tarihsel kayıt olarak korunur; güncel hedefe tekrar uygulanmaz.
...
## Güncel paket ve proje erişimi

Canonical ZIP, `0000–0027` zincirini içerir: 28 migration, journal dışında 36 uygulama tablosu, journal dahil 37 tablo. Aşağıdaki `0024` sayıları eski sürüme aittir. Güncel geçişin yedek/restore, hedef doğrulama ve iki DDL ifadesi [0027 runbook'unda](./phpmyadmin-project-access-incremental.md) tanımlıdır. Proje erişimi için `npm run test:project-access:mariadb` dar doğrulama komutudur; aynı altı kontrol zorunlu tam MariaDB yayın kontrolüne de dahildir.

## Tarihsel 0024 DB-first Hostinger yükleme akışı
...
5. DB adımı PASS olduktan sonra yalnız bu paketle aynı final committen üretilmiş canonical uygulama artefaktı kullanılır. Hostinger Git bağlantısı otomatik dağıtıyorsa commit `main`e ancak bu noktada birleştirilir. Manuel ZIP akışında yalnız `dist/portal-pusula-hostinger.zip` kullanılır. Kullanıcının 17 Eylül 2026 tarihli açık talebiyle Codex yüklemeyi ve dağıtımı mevcut ayarları koruyarak tamamlayabilir. Parolayı göster düğmeleri kullanılmaz, input değerleri DOM/erişilebilirlik çıktısından alınmaz. Panel mevcut değerleri kendiliğinden açık gösterirse değerler kopyalanmadan, değiştirilmeden ve tekrar görüntülenmeden yalnız yükleme/dağıtım kontrolüyle devam edilir; mevcut değeri korumak yeni kimlik bilgisi girişi değildir. Yeni kimlik bilgisi girişi kullanıcı tarafından yapılır; genel kurulum onayı tekrar istenmez.
...
Gerçek sırlar sohbet/belgeye istenmez veya aktarılmaz. Hostinger manuel yeniden dağıtımında mevcut değerler korunur; input değerleri DOM'dan çıkarılmaz ve sır gösterme kontrolleri kullanılmaz. Panelin kendiliğinden açık değer göstermesi durumunda daha fazla görüntüleme yapılmaz; yalnız mevcut yapılandırmayı değiştirmeyen dağıtım tamamlanabilir. Yeni sır oluşturma/girme veya rotasyon kullanıcı tarafından yapılır. `DB_NAME`, `DB_USER`, `DB_PASSWORD` ve tam 16 karakterlik, yalnız ASCII `A-Z`/`a-z`/`0-9` içeren rastgele readiness token eksik veya biçim dışıysa sınır fail-closed çalışır. 15/17 karakter, boşluk, Türkçe/özel karakter ve semboller kabul edilmez.
