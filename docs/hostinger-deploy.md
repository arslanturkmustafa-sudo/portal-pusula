...
Bu runbook, `portal.muhendiskafasi.com.tr` Hostinger Business Node.js Web App hedefini ve güncel operasyon sürümünü kapsar. Bu sürümün DB-first başlangıç koşulu, canlı journal'ın exact 16 kayıtla `0015_financial_reversals` seviyesinde olmasıdır. Daha eski hedef önce korunmuş sürümlü incremental runbook'larıyla exact `0015` seviyesine getirilir. Ardından hedefe bağlı iki ayrı ve tek kullanımlık paket araya uygulama deploy edilmeden sırasıyla `0016_finance_accounts_ledger` ve `0017_work_task_visit` için uygulanır; final journal exact 18 kayıttır.
...
Uygulama ZIP'i DB şeması uygulamaz. npm/SSH erişimi olmayan, yeni ve tamamen boş disposable staging DB'si için [phpMyAdmin clean-only migration runbook'u](./phpmyadmin-clean-migration.md) kullanılır. Journal'ı bulunan eski hedefler `0011`–`0015` için kendi tarihsel runbook'larıyla exact `0015` seviyesine getirilir; [0014/0015 yaşam döngüsü ve ters kayıt incremental runbook'u](./phpmyadmin-lifecycle-reversals-incremental.md) değiştirilmeden korunur. Güncel canlı geçişte [0016/0017 finans hesabı ve görev–ziyaret incremental runbook'u](./phpmyadmin-finance-calendar-incremental.md) uygulanır. Üretilmiş clean/incremental phpMyAdmin bundle artefaktları canonical Hostinger ZIP'ine girmez; sürümlü Drizzle migration SQL'leri ise doğrulama ve ileriye dönük bakım için paketlenir ancak install/build/start sırasında otomatik uygulanmaz.

Arşiv kökünde doğrudan `package.json`, `package-lock.json`, `.nvmrc`, `next.config.mjs`, `next-env.d.ts`, `tsconfig.json` ve `postcss.config.mjs` bulunur. `drizzle/` içindeki exact `0000`–`0017` zinciri, production için gereken üç migration scripti ve testleri çıkarılmış `src/` sabit allowlist ile dahil edilir. Clean zincir exact 18 migration, 18 journal kaydı ve journal dışında 28 uygulama tablosudur. Public allowlist yalnız Mühendis Kafası logosu, üç PWA ikonu ve `offline-v1.html` dosyasıdır; `/sw.js` public dosya değil Node Route Handler'dır. `src/**/*.test.ts(x)`, `tests/`, disposable DB/E2E/paketleme scriptleri, `.env*`, özel anahtar/credential dosyaları, `.next/`, `node_modules/`, `dist/`, `outputs/`, `work/`, log/coverage/Playwright/test çıktıları ve Git verisi hariçtir. Nested yasak yol bulunduğunda paket içerik üretmeden fail-closed olur. Hostinger'a yüklenen ZIP'in kökünde `package.json` doğrudan görünmelidir; arşiv bir üst klasör içine sarılmaz.
...
3. Uygulama yazmaları ön kontrolden deploy sonrası smoke testleri bitene kadar dondurulur. Başlangıç journal'ı exact 16 satır ve son kayıt `0015_financial_reversals` değilse durulur; daha eski hedef önce kendi tarihsel runbook'larıyla exact `0015` seviyesine getirilir.
4. [0016/0017 incremental runbook'u](./phpmyadmin-finance-calendar-incremental.md) ile canlı DB/server digest'lerine bağlı `0016_finance_accounts_ledger` paketi üretilir, manifest ve SQL SHA-256 doğrulanır ve phpMyAdmin'de yalnız bir kez uygulanır. Exact başarı satırı, 17 journal kaydı, journal dışında 27 uygulama tablosu, üç boş finans tablosu ve exact 37 izin kodu postflight'ı PASS olmadan ilerlenmez.
5. Araya uygulama deploy etmeden ve aynı hedef digest'leri korunarak ayrı `0017_work_task_visit` paketi üretilir, manifest ve SQL SHA-256 doğrulanır ve yalnız bir kez uygulanır. Exact başarı satırı, final 18 journal kaydı, journal dışında 28 uygulama tablosu ve boş ilişki tablosunun PK/FK/CHECK/index postflight'ı PASS olmadan uygulama deploy edilmez. Exact başarı yoksa aynı SQL tekrar çalıştırılmaz; hedef salt okunur incelenir ve gerekirse yeni forward-fix hazırlanır.
...
9. Dağıtım `Akım` olduktan sonra liveness/readiness, giriş/kullanıcı yetkileri, alan redaksiyonu, yaşam döngüsü, void/ters kayıt, audit geçmişi, `/finans/hesaplar`, firma görev raporu, nakit akışı ve günlük/haftalık/aylık plandaki görev–ziyaret bağları doğrulanır; yalnız sonra yazma dondurması kaldırılır.
...
- Güncel uygulama `0016_finance_accounts_ledger` ile `0017_work_task_visit` şemasına bağımlıdır; final journal exact 18 olmadan bu build dağıtılmaz.
- `0014`–`0017` forward-only DDL'dir. Eski uygulama yeni lifecycle/void/reversal, hesap ledger'ı ve görev–ziyaret sözleşmelerinin tümünü bilmediği için sıradan bir uygulama rollback'i güvenli sayılmaz; yazma trafiği şema uyumlu uygulama yeniden etkin olana kadar kapalı kalır.
...
- Önceki uygulama artefaktı yalnız canonical paket sözleşmesiyle yeniden üretilmiş, secret-safe biçimde korunmuş, final 0017 şemasıyla uyumluluğu doğrulanmış ve aynı MariaDB session güvenlik sözleşmesini taşıyorsa geri dönüş adayıdır. Benzer adlı eski ZIP veya spike artefaktı bu koşulu sağlamaz.
...
- [ ] Yazma trafiği donduruldu; başlangıç journal'ı exact 16 satır ve son kayıt `0015_financial_reversals`; üç `0016` tablosu ile `work_task_visit` yok ve izin CHECK'i exact 35 kod.
- [ ] Hedefe bağlı `0016_finance_accounts_ledger` paketi exact başarıyla tamamlandı; journal 17, journal dışında 27 uygulama tablosu, üç boş finans tablosu ve exact 37 izin kodu doğru.
- [ ] Araya uygulama deploy edilmeden hedefe bağlı `0017_work_task_visit` paketi exact başarıyla tamamlandı; final journal 18, journal dışında 28 uygulama tablosu, boş ilişki tablosunun PK/FK/CHECK/index sözleşmesi ve exact 37 izin kodu doğru.
...
- [ ] `/finans/hesaplar` üzerinde kasa/banka açılış bakiyesi, gelir/gider/transfer, çift taraflı ledger bakiyesi, pasife alma ve ileri yönlü ters kayıt izin sınırları doğru.
...
- [ ] Günlük planın günlük/haftalık/aylık görünümü doğru aralığı gösteriyor; görevler takvim gününe ve uygun planlı ziyarete doğru bağlanıyor, ziyaret tamamlama iş maddeleri yetki sınırlarıyla kaydediliyor.
