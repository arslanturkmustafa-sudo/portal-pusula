...
Bu runbook, `portal.muhendiskafasi.com.tr` Hostinger Business Node.js Web App hedefini ve güncel operasyon sürümünü kapsar. Bu sürümün DB-first başlangıç koşulu, canlı journal'ın exact 23 kayıtla `0022_recurring_tasks_expenses` seviyesinde olmasıdır. Daha eski hedef önce korunmuş sürümlü incremental runbook zinciriyle exact `0022` seviyesine getirilir. Ardından hedefe bağlı ve tek kullanımlık `0023_collection_card_accounts` paketi uygulama deploy edilmeden uygulanır; final journal exact 24 kayıttır.
...
3. Uygulama yazmaları ön kontrolden deploy sonrası smoke testleri bitene kadar dondurulur. Başlangıç journal'ı exact 23 satır, `MAX(id)=23`, `AUTO_INCREMENT=24` ve son kayıt kaynak zincirindeki `0022_recurring_tasks_expenses` kimliğiyle (`created_at=1789060097371`, `hash=9aca25ba6a18ee00a0cedb32f8e5a6ea22b2dfdb813302b9a87bffc3f03cbf5f`) eşleşmiyorsa durulur. `credit_card_installment.finance_transaction_id` veya `receivable_collection.finance_transaction_id` önceden varsa da durulur; daha eski hedef önce kendi tarihsel runbook'larıyla exact `0022` seviyesine getirilir.
4. Canlı DB/server SHA-256 digest'lerine bağlı paket `PHPMYADMIN_INCREMENTAL_MIGRATION_TAG=0023_collection_card_accounts`, `PHPMYADMIN_TARGET_DB_SHA256` ve `PHPMYADMIN_SERVER_VERSION_SHA256` process environment adlarıyla `npm run db:bundle:phpmyadmin:incremental` çalıştırılarak üretilir. Manifest; `expectedJournalCount=23`, önceki migration `0022_recurring_tasks_expenses`, sekiz statement, migration `createdAt=1789195799696` ve `hash=e4a5e27b1b6113baa0bafcc52b29c7d1e8a78a293ee326daf5eb086b48bed5e3` değerlerini göstermeli; manifest SQL SHA-256 değeri gerçek dosya hash'iyle eşleşmelidir. Yalnız `dist/portal-pusula-incremental-0023_collection_card_accounts.sql` phpMyAdmin'de bir kez uygulanır. Exact `PORTAL_PUSULA_INCREMENTAL_MIGRATION_OK` / `0023_collection_card_accounts` sonucu, final journal 24 ve journal dışında 32 uygulama tablosu doğrulanmadan uygulama deploy edilmez. Postflight'ta iki yeni kolon nullable `char(36) CHARACTER SET ascii COLLATE ascii_bin`; `uq_credit_card_installment_finance_transaction`, `uq_receivable_collection_finance_transaction`, `chk_credit_card_installment_finance_transaction`, `chk_receivable_collection_finance_transaction_identity`, `fk_credit_card_installment_finance_transaction` ve `fk_receivable_collection_finance_transaction` olmak üzere exact altı constraint bulunmalı; iki FK de `finance_transaction(id)` hedefine `ON DELETE RESTRICT ON UPDATE RESTRICT` bağlanmalıdır. Exact başarı yoksa aynı SQL tekrar çalıştırılmaz; hedef salt okunur incelenir ve gerekirse yeni forward-fix hazırlanır.
...
- Güncel uygulama `0023_collection_card_accounts` şemasına bağımlıdır; final journal exact 24 olmadan bu build dağıtılmaz.
- `0014`–`0023` forward-only DDL'dir. Eski uygulama yeni lifecycle/void/reversal, hesap ledger'ı, görev–ziyaret, konum/gider kategori, bildirim/tekrar ve tahsilat–kart hareket bağlantısı sözleşmelerinin tümünü bilmediği için sıradan bir uygulama rollback'i güvenli sayılmaz; yazma trafiği şema uyumlu uygulama yeniden etkin olana kadar kapalı kalır.
...
- Önceki uygulama artefaktı yalnız canonical paket sözleşmesiyle yeniden üretilmiş, secret-safe biçimde korunmuş, final 0023 şemasıyla uyumluluğu doğrulanmış ve aynı MariaDB session güvenlik sözleşmesini taşıyorsa geri dönüş adayıdır. Benzer adlı eski ZIP veya spike artefaktı bu koşulu sağlamaz.
...
- [ ] Yazma trafiği donduruldu; başlangıç journal'ı exact 23 satır, `MAX(id)=23`, `AUTO_INCREMENT=24` ve son kayıt `0022_recurring_tasks_expenses` kimliğiyle eşleşiyor; `credit_card_installment.finance_transaction_id` ile `receivable_collection.finance_transaction_id` henüz yok.
- [ ] Hedefe bağlı `0023_collection_card_accounts` paketi exact başarıyla tamamlandı; final journal 24, `MAX(id)=24`, `AUTO_INCREMENT=25`, id 24 kaydı `created_at=1789195799696` ve `hash=e4a5e27b1b6113baa0bafcc52b29c7d1e8a78a293ee326daf5eb086b48bed5e3`; journal dışında 32 uygulama tablosu, iki nullable ASCII `char(36)` kolon, iki unique, iki check ve `finance_transaction(id)` hedefine bağlı iki RESTRICT FK doğru.
